...
import { RankBadge, Streak } from '../components/UI';
import { METAL } from '../components/constants';
  ...
  return (
    <button onClick={() => setI((i + 1) % CLIMATE_FACTS.length)} aria-label="Show another climate fact"
      style={{ width: '100%', textAlign: 'left', cursor: 'pointer', border: '1px solid rgba(46,125,79,.18)', borderRadius: 18,
        padding: 16, background: 'radial-gradient(180px 80px at 90% -10%, rgba(117,183,123,.14), transparent), var(--navy-800)',
      ...
      <div style={{ flex: 1 }}>
        ...
        <div className="muted" style={{ fontSize: 14, fontWeight: 600, lineHeight: 1.4, color: 'var(--text)' }}>{CLIMATE_FACTS[i]}</div>
      </div>
    </button>
  );
}
    ...
    <div className="screen-in">
      ...
      <div style={{ padding: '16px 16px 0' }}>
        <button onClick={() => go('coach')} aria-label="Open the AI Eco Coach"
          style={{ width: '100%', textAlign: 'left', cursor: 'pointer', border: '1px solid rgba(46,125,79,.22)', borderRadius: 18,
            padding: 16, background: 'radial-gradient(180px 90px at 88% -20%, rgba(117,183,123,.18), transparent), linear-gradient(180deg,var(--navy-800),var(--navy-700))',
            display: 'flex', gap: 13, alignItems: 'center' }}>
          <span style={{ width: 46, height: 46, borderRadius: 14, flexShrink: 0, background: 'rgba(46,125,79,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Icon name="sparkle" size={24} color="var(--green)" />
          </span>
          <div style={{ flex: 1 }}>
            <div className="eyebrow" style={{ color: 'var(--green)', marginBottom: 3 }}>AI Eco Coach · new</div>
            <div style={{ fontFamily: 'var(--display)', fontWeight: 600, fontSize: 16, color: 'var(--text)' }}>Ask the Eco Coach</div>
        ...
        <div className="card" style={{ padding: '10px 8px 12px' }}>
          {top3.length >= 3 ? (
            ...
            <div style={{ display: 'grid', gap: 4 }}>
              {top3.map(p => (
                <div key={p.user_id} style={{ display: 'flex', alignItems: 'center', gap: 11, padding: '8px 8px', borderTop: '1px solid rgba(45,91,57,.08)' }}>
      ...
      <div style={{ padding: '18px 16px 0' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <button onClick={scrollToQuests} className="card" style={{ textAlign: 'left', cursor: 'pointer', border: 'none', padding: 16, display: 'flex', flexDirection: 'column', gap: 10 }}>
            ...
            <span style={{ fontFamily: 'var(--display)', fontWeight: 600, fontSize: 15, color: 'var(--text)' }}>Daily quests</span>
