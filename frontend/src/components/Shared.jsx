...
function PhotoRow({ icon, label, sub, accent, onClick }) {
  return (
    <button onClick={onClick} className="card" style={{
      ...
      <span style={{ flex: 1 }}>
        <span style={{ display: 'block', fontFamily: 'var(--display)', fontWeight: 700, fontSize: 16, color: 'var(--text)' }}>{label}</span>
  ...
  return (
    <>
      ...
      <div className="screen-in" style={{
        ...
        background: 'linear-gradient(180deg,#fff,var(--navy-900))', border: `1px solid ${accent}33`,
        boxShadow: '0 -16px 50px rgba(30,91,57,.18)',
      }}>
    ...
    <>
      ...
      <div className="sheet" role="dialog" aria-modal="true" aria-label={title} tabIndex={-1} ref={sheetRef}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 20px 10px', flexShrink: 0 }}>
          ...
          <button className="btn btn-secondary btn-sm" style={{ padding: 8 }} onClick={onClose} aria-label="Close"><Icon name="x" size={18} aria-hidden="true" /></button>
        </div>
...
export function UploadFrame({ phase, label, accent = 'var(--green)', onCapture }) {
  return (
    <div onClick={phase === 'capture' ? onCapture : undefined} style={{
      ...
      {phase === 'capture' && (
        <>
          <span style={{ width: 60, height: 60, borderRadius: '50%', background: 'rgba(46,125,79,.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      ...
      {phase === 'analyzing' && (
        <>
          <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg,#2E7D4F,#75B77B)' }} />
      ...
      {phase === 'result' && (
        <div style={{ position: 'absolute', bottom: 12, left: 12, display: 'flex', alignItems: 'center', gap: 8, background: 'rgba(0,0,0,.45)', padding: '8px 14px', borderRadius: 9999, backdropFilter: 'blur(6px)' }}>
          <span style={{ width: 22, height: 22, borderRadius: '50%', background: 'var(--green)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Icon name="check" size={14} color="#fff" strokeWidth={3} /></span>
    ...
    <svg width={size} height={size} viewBox="0 0 64 64" fill="none">
      <defs>
        <linearGradient id="lgM" x1="10" y1="6" x2="54" y2="58" gradientUnits="userSpaceOnUse">
          <stop stopColor="#75B77B" /><stop offset="1" stopColor="#2E7D4F" />
        </linearGradient>
      </defs>
      <rect x="4" y="4" width="56" height="56" rx="18" fill="#F7FAF5" stroke="rgba(46,125,79,.28)" strokeWidth="1.5" />
      <path d="M20 44c0-15 10-23 24-24-1 15-9 23-24 24Z" fill="url(#lgM)" />
      <path d="M20 44c3.5-8 8.5-12 15-15" stroke="#F7FAF5" strokeWidth="2.4" strokeLinecap="round" opacity=".72" />
      <path d="M40 16l2.4 6.6L49 25l-6.6 2.4L40 34l-2.4-6.6L31 25l6.6-2.4L40 16Z" fill="#C6A35A" />
    </svg>
  );
}

// ── Wordmark ──
export function Wordmark({ size = 26, color = 'var(--text)' }) {
  return (
    <span style={{ fontFamily: 'var(--display)', fontWeight: 700, fontSize: size, letterSpacing: 0, color, lineHeight: 1 }}>
      Eco<span style={{ color: 'var(--green)' }}>Rise</span>
    </span>
  );
}

// ── Botanical paper backdrop ──
export function Orbs() {
  return (
    <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', pointerEvents: 'none' }}>
      <div className="floaty" style={{ position: 'absolute', top: 34, left: -70, width: 260, height: 260, borderRadius: '50%', background: 'radial-gradient(circle,rgba(117,183,123,.18),transparent 68%)', opacity: .9, filter: 'blur(10px)' }} />
      <div className="floaty" style={{ position: 'absolute', bottom: 40, right: -85, width: 300, height: 300, borderRadius: '50%', background: 'radial-gradient(circle,rgba(198,163,90,.12),transparent 68%)', opacity: .9, filter: 'blur(12px)', animationDelay: '1.2s' }} />
      <div style={{ position: 'absolute', inset: 0, backgroundImage: 'linear-gradient(rgba(30,91,57,.045) 1px, transparent 1px), linear-gradient(90deg, rgba(30,91,57,.035) 1px, transparent 1px)', backgroundSize: '28px 28px', maskImage: 'linear-gradient(180deg, rgba(0,0,0,.8), transparent 80%)' }} />
    </div>
  );
}
...
export function ResetTimer({ target }) {
  ...
  const cell = (v, l) => (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minWidth: 40 }}>
      <span style={{ fontFamily: 'var(--display)', fontWeight: 700, fontSize: 22, color: 'var(--text)', fontVariantNumeric: 'tabular-nums' }}>{String(v).padStart(2, '0')}</span>
  ...
  return (
    <div className="card" style={{ padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        <span className="eyebrow" style={{ color: 'var(--coral-d)' }}>Resets in</span>
      </div>
