...
function writeUniquePng(filePath) {
  const baseBuffer = Buffer.from(DUMMY_PNG_BASE64, 'base64');
  const uniqueBuffer = Buffer.concat([baseBuffer, crypto.randomBytes(16)]);
  fs.writeFileSync(filePath, uniqueBuffer);
}

writeUniquePng(tempImagePath);
...
async function run() {
  ...
  try {
    ...
    // Click submit/post button to trigger the Evidence Panel if visible
    const postBtn = page.locator('text=Post & earn points');
    if (await postBtn.isVisible()) {
      console.log('Submitting verification...');
      await postBtn.click();
      await page.waitForTimeout(5000);
    } else {
      console.log('No follow-up needed, transitioned directly to Evidence Panel.');
    }
