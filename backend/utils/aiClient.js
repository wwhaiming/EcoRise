...
const MOCK_QUESTS = [
  ...
  { title: 'Single-Use Free Day', description: 'Go a full school day with no single-use plastic', actionType: 'waste', targetDetails: 'no single-use plastic for a day', pointsBase: 40 },
...
Rules: at least 2 quests target the neglected categories; at least 1 builds on their most frequent; vary across transportation, waste, energy, water, nature; each completable in a day with a concrete, photo-verifiable targetDetails; actionType in [transportation, waste, energy, water, nature, community]; pointsBase integer 40-80; goal integer 1-3 (most quests are 1; a repeat/streak quest like "refill 3 times" may be 2-3, each step needing its own verified photo). Respond ONLY in JSON.` }],
      response_format: { type: 'json_object' },
    });
    const json = extractJson(response.choices[0].message.content);
